...
PROJECTS = [
    ...
    },
    {
        ...
        "description": "用語音或自然語言描述食材，Google Gemini AI 自動解析名稱、數量、到期日，並可根據庫存食材 AI 推薦食譜。支援左滑刪除、拖拉排序與深色模式。",
        "description_en": "Describe food items by voice or text; Gemini AI auto-parses name, quantity, and expiry. Also suggests recipes based on inventory. Features swipe-to-delete, drag-to-reorder, and dark mode.",
        "details": "家庭食材庫存管理 Web App，以語音輸入為核心設計。後端採 Flask + Flask-SQLAlchemy，Neon PostgreSQL 存資料，Render 免費方案部署。AI 解析由 Google Gemini 2.5 Flash 驅動，失敗時自動降級到 jieba + regex 本地方案，確保無 API Key 時仍可使用。支援批次新增（一句話描述多項食材）、到期色碼預警、手機左滑刪除、長按拖拉排序存放地點，以及系統 / 亮 / 暗三段切換的深色模式，多使用者帳號完全隔離。最新新增 AI 食譜建議功能：可指定食材或結合現有庫存，由 Gemini 自動推薦適合食譜。",
        "details_en": "A food pantry web app centered on voice input. Built with Flask + Flask-SQLAlchemy on Neon PostgreSQL, deployed on Render free tier. AI parsing is powered by Google Gemini 2.5 Flash with automatic fallback to a local jieba + regex pipeline, keeping the app functional at zero cost. Features batch item entry, expiry color-coded alerts, mobile swipe-to-delete, drag-to-reorder, three-mode dark theme, and full per-user data isolation. Latest addition: AI recipe suggestions — specify ingredients manually or combine with existing inventory for Gemini-powered recipe recommendations.",
        "highlights": [
            "Gemini 2.5 Flash AI 語意解析，失敗自動降級到本地 jieba + regex",
            "瀏覽器原生 Web Speech API 語音輸入，支援 iOS Safari & Chrome",
            "批次新增：一句話描述多項食材，預覽確認後一鍵加入庫存",
            "AI 食譜建議：指定食材或結合現有庫存，Gemini 自動推薦食譜",
        ...
        "highlights_en": [
            ...
            "AI recipe suggestions: specify ingredients or combine with existing inventory for Gemini-powered recommendations",
    ...
    },
    {
        ...
        "details": "整合 LINE Messaging API 的政府職缺訂閱通知機器人。使用者透過分頁 Quick Reply 多選工作地點、職系細項，並輸入職缺關鍵字後，傳送任何訊息即可即時爬取人事行政總處（DGPA）網站，取得最近 30 天符合條件的職缺並以手機優化格式回覆。後端以 FastAPI 建構，BeautifulSoup 解析 ASP.NET WebForms 頁面（含 __VIEWSTATE 跨頁 POST），訂閱設定存於 Neon PostgreSQL，本機自動 fallback 到 SQLite，部署於 Render 免費方案。",
        "details_en": "A LINE chatbot for subscribing to Taiwan DGPA government job listings. Users set their work location and job category via paged Quick Reply multi-select, then any message triggers a real-time crawl of the DGPA site, returning matching vacancies from the past 30 days in a mobile-optimized format. Built on FastAPI; BeautifulSoup handles ASP.NET WebForms pagination including __VIEWSTATE multi-page POST. Subscriptions stored in Neon PostgreSQL with automatic SQLite fallback for local dev. Deployed on Render free tier.",
        "highlights": [
            "BeautifulSoup 爬取 ASP.NET WebForms 分頁，自動處理 __VIEWSTATE 跨頁 POST",
            "地點與職系細項採分頁 Quick Reply 多選，支援 ✅ 切換與舊版文字輸入 fallback",
        ...
        "highlights_en": [
            ...
            "Location & job category use paged Quick Reply multi-select with ✅ toggle and text-input fallback",
